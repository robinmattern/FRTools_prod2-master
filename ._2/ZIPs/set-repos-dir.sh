...
#    aZIP_Ver="$( 7zip -h 2>&1 | awk '/^7-Zip/ { print $3 }' )";                                                              ##.(50102.04.3)
     aZIP_Ver="$( zip   -h 2>&1 | awk '/Info-ZIP/    { print "Info-Zip zip " $2 }' )";     if [ "${aZIP_Ver}" == "" ]; then   # .(50102.04.3 RAM Check for many version of 7zip Beg)
     ...
     aZIP_Ver="$( 7zip  -h 2>&1 | awk '/\(c\).+Igor/ { print "7-Zip  7zip "  $3 }' )"; fi; if [ "${aZIP_Ver}" == "" ]; then  
     aZIP_Ver="$( 7-zip -h 2>&1 | awk '/\(c\).+Igor/ { print "7-Zip  7-zip " $3 }' )"; fi;                                    # .(50102.04.3 End
