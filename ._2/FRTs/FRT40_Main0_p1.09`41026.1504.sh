...
#    aFns="$( dirname "${BASH_SOURCE}"         )/FRT12_Main2Fns_p1.06.sh";     if [ ! -f "${aFns}" ]; then  ##.(21113.05.9 RAM Use FRT12_Main2Fns_p1.06_v21027.sh)
     aFns="$( dirname "${BASH_SOURCE}"         )/JPT12_Main2Fns_p1.07.sh";     if [ ! -f "${aFns}" ]; then  # .(41026.04.1 RAM Use copy in FRTs).(21113.05.9 RAM Use FRT12_Main2Fns_p1.06_v21027.sh)
#    aFns="$( dirname "${BASH_SOURCE}" )/../JPTs/JPT12_Main2Fns_p1.07.sh";     if [ ! -f "${aFns}" ]; then  ##.(41026.04.1)
...
#       aJPT_dir="$( dirname $0 )";              echo "Before: '${aJPT_dir}'"                               # .(41026.05.1)
        aJPT_dir="$( dirname $0 )"; aJPT_dir="$( echo "${aJPT_dir}" | awk '{ sub( /FRT/, "JPT" ); sub( "/._2", "" ); print }' )"  # .(41026.05.2)
#                                                echo "After:  '${aJPT_dir}'"; exit                         # .(41026.05.3)
...
#       "$( dirname $0 )/../JPTs/JPT00_Main0.sh"  ${aSubCmd}  "$@"                                          # .(41026.05.6)
        "${aJPT_dir}/JPT00_Main0.sh"  ${aSubCmd}  "$@"

   aJPT_dir="C:/Home/Shared/Repos/FRTs_/prod1-master/._2/JPT00_Main0.sh"
   aJPT_dir="$( echo "${aJPT_dir}" | awk '{ sub( /FRT/, "JPT" ); sub( "\._2/", "" ); print }' )"
...
#       sayMsg "FRT10[322]  gitR Commands (${aArg1:0:3}) aCmd: '${aCmd}'" 1                                 # .(20429.03.2 Beg RAM Added)
     ...
     if [ "${aCmd}"    ==  "gitR" ]; then
        ...
        sayMsg "FRT10[326]  gitR:  '${aArg1}' '${aArg2}' '${aArg3}' '${aArg4}', bDoit: '${bDoit}', bDebug: '${bDebug}', bQuiet: '${bQuiet}'" -1
...
#       sayMsg "FRT10[348]  dokS Commands (${aArg1:0:3})"                                                   # .(20429.02.2 Beg RAM Added)
     ...
     if [ "${aCmd}"     == "dokR" ]; then
        ...
        shift
       "$( dirname $0 )/dokR/FRT25_dokR1.sh"  "$@"                                                          # .(41026.03.3 RAM Was FRT24_)
