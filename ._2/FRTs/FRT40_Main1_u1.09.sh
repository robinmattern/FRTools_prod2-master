...
#  # echo "         appR Deploy"                                                        # .(20411.08.1 RAM Add)
   ...
   # echo ""
     ...
     echo "    FRT New Repo {RepoName} [-doit]      Create a new local repo folder"                         # .(50816.01.1 RAM).(41218.04b.1).(41218.04.1)
     echo "    FRT Clone    {RepoName}              Clone a remote Github repo"                             # .(50816.01.2).(50105.06.2)
     echo "    FRT Copy     {FromBr} {File} {ToBr}  Copy file from to a branch"                             # .(41225.06.1)
     echo ""
...
#====== =================================================================================================== #
  ...
  if [ "${aCmd}" == "Init" ]; then                                                                          # .(41218.04.3 RAM Add Init Command Beg)
        shift
  if [ "$1" == "" ]; then
...
#       echo -e "\n* Please provide repo to install, e.g. ALTools or AIDocs."                               # .(41124.02.4)
        ...
        echo "  You can also install a repo, e.g. AICodeR or AIDocs."; ${aLstSp}; fi                        # .(50104.01.1 RAM Chg ALTools to AICodeR)
        exit
...
#            fi    # eif ???
                                              echo "    frt copyFile \"ALTools\" \"run-anyllm.sh\" \"master\" -doit" # .(41225.06.3)
...
# ----  ---------------------------------------------------------------------  ----
        ...
        echo -e "\n* Please provide a valid repo to install, e.g. AICodeR or AIDocs."                       # .(50104.01.7).(41124.04.3)
