...
##SRCE     +====================+===============================================+
#*/
          aVdt="May 20, 2023  5:16p"; aVtitle="OS Info Tools"
...
#   +===== +================== +=========================================================== # ==========+
     ...
     if [ "${aCmd1:0:3}" == "pro" ]; then aCmd1=profile; aCmd2="save"; aArg1=""; fi          # .(30520.02.1 RAM Add Save Profile)
     if [ "${aCmd2:0:3}" == "pro" ]; then aCmd1=profile; aCmd2="save"; aArg1=""; fi          # .(30520.02.2)
     if [ "${aArg1:0:3}" == "pro" ]; then aCmd1=profile; aCmd2="save"; aArg1=""; fi          # .(30520.02.3)

     if [ "${aCmd1:0:3}" == "ser" ]; then aCmd1=vars; aCmd2="show"; aArg1=server; fi         # .(30520.01.1 RAM Add Show|Set server)
     if [ "${aCmd2:0:3}" == "ser" ]; then aCmd1=vars; aCmd2="show"; aArg1=server; fi         # .(30520.01.2)
     if [ "${aArg1:0:3}" == "ser" ]; then aCmd1=vars; aCmd2="show"; aArg1=server; fi         # .(30520.01.3)
...
#           echo "  $LIB Info Functions              $( echo $0 | awk '{ gsub( /.+_[ptuv]|.sh/, ""); print }' )"
            ...
            echo "    RSS Info Save Profile [-doit]             Save ~\.bash_profile for user: ${aUser}"    # .(30520.02.1)
...
#====== =================================================================================================== #
     ...
     if [ "${bDoit}" != "1" ]; then
            ...
            echo "--------------------------------------------------------------------------------------------"
            cat  "$(dirname $0)/RSS25_.bash_profile"
            echo "--------------------------------------------------------------------------------------------"
...
#           cp  "./Info/RSS25_.bash_profile" ~/.bash_profile
            ...
            echo -e   "        and then set it in the ~/.bash_profile file."
            fi
...
#                                aVerb="will be"; aToDo=""                                               # .(21114.02.14).(30520.03.5)
                                 aVerb="needs to be"; aToDo=""                                           # .(30520.03.5 RAM Will be?)
         fi                                                                                              # .(21114.02.15)
#    +---- +------------------ +-----------------------------------------------
...
#====== =================================================================================================== #
     ...
     if [ "$aArg1"       == "os"  ]; then aArg1="OS"; bSkip=1; else bSkip=0; fi             # .(21126.04.2 RAM).(30520.01.6 RAM)
     if [ "${aArg1:0:3}" == "ser" ]; then             bSkip=1; fi                           # .(21126.04.2 RAM).(30520.01.7 RAM)
...
#          echo ""
     ...
     if [ "${aArg2}" != "" ]; then
            ./info/RSS24_setServer.sh "$4" "$5" "$6" "$7" "$8"
            bCmdRan=1
           fi                                                                               # .(30520.01.9 End)
        fi
# ------------------------------------------------------------------------------------
